...
#define SERVO_CENTER   16680 //8340// 33360 //4170            /* サーボのセンタ値             */
...
void main( void )
{
    ...
    while( 1 ) {
        ...
        if( i == 1 ) {
            ...
            case 'z':
                servo_offset++;
                if( servo_offset > 20000 ) servo_offset = 20000;
                printf( "%5d\r", servo_offset );
                break;
...
void intTRB( void )
{
	static int servo_cnt = 0,servo_angle = 0,hoge = 0;
	...
	hoge ++;
	if(hoge < 250)led_out(1);
	else if(hoge < 500)led_out(2);
	else hoge = 0;	
}
